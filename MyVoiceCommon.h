...
// ============================================================
// ★ 2.2.10：内置方言音色（CosyVoice「声音设计」生成，听感更真人）
//
// 千问预置里没有阳江话/皖北话这类小方言，所以用 CosyVoice 的 voice_prompt
// （文字描述音色）来生成。每个方言给男女两版，点选时首次会调用 designVoiceWithName:
// 生成专属 voice_id（约 10~30 秒），之后写进共享配置长期可用。
//   阳江话 → 阳江白话（粤语阳江片）；皖北话 → 皖北中原官话（阜阳一带）。
// ============================================================
static inline NSArray* MVCosyDialectList(void) {
    return @[
        @{@"name": @"阳江话·男声", @"voiceKey": @"yangjiang_m", @"model": @"cosyvoice-v3.5-plus",
          @"design": @YES,
          @"prompt": @"一个广东阳江的年轻男性，讲地道阳江白话（粤语阳江片），口语自然随性，语速适中，带点市井烟火气，像邻家阿哥闲聊。",
          @"preview": @"今日天气唔错，我哋去饮茶啦，叫笼虾饺烧卖都好。"},
        @{@"name": @"阳江话·女声", @"voiceKey": @"yangjiang_f", @"model": @"cosyvoice-v3.5-plus",
          @"design": @YES,
          @"prompt": @"一个广东阳江的年轻女子，讲阳江白话（粤语阳江片），声音清甜亲切，说话像跟老友倾偈，自然不生硬。",
          @"preview": @"喂，食咗饭未啊，得闲出来行下啦。"},
        @{@"name": @"皖北话·男声", @"voiceKey": @"wanbei_m", @"model": @"cosyvoice-v3.5-plus",
          @"design": @YES,
          @"prompt": @"安徽阜阳一带的北方中年男性，讲皖北中原官话，口音醇厚朴实，咬字清楚，语速平稳，像村里长辈拉家常。",
          @"preview": @"今个天晴得好，咱一块上集去，买点菜回来烧饭。"},
        @{@"name": @"皖北话·女声", @"voiceKey": @"wanbei_f", @"model": @"cosyvoice-v3.5-plus",
          @"design": @YES,
          @"prompt": @"安徽皖北的年轻女子，讲皖北中原官话（阜阳口音），嗓音温和大方，说话实在利落，带点乡土亲切感。",
          @"preview": @"俺刚从地里回来，你吃饭了没，咱俩唠唠。"},
    ];
}

// 方言音色生成后落盘的「voiceKey → 真实 voice_id」映射（切引擎/重开都还在）
static inline NSString* MVDialectRealVoiceID(NSString *voiceKey) {
    NSDictionary *d = MVGet(@"dialectVoices");
    if ([d isKindOfClass:[NSDictionary class]]) return d[voiceKey];
    return nil;
}
static inline void MVSetDialectRealVoiceID(NSString *voiceKey, NSString *vid) {
    if (!voiceKey.length || !vid.length) return;
    NSMutableDictionary *d = [NSMutableDictionary dictionaryWithDictionary:
        (MVGet(@"dialectVoices") ?: @{})];
    d[voiceKey] = vid;
    MVSetShared(@"dialectVoices", d);
}
