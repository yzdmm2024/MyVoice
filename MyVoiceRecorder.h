...
//   2.2.0 起每个 AudioQueue 输入队列独立登记（回调 + 采样率），替换与转发按队列精确匹配；
//   实际消费队列的采样率在回调时才知道，届时做一次性自适应重采样。
//   （修复：旧版只绑定进程里第一个录音队列，音色克隆的 AVAudioRecorder 若先建队列
//     会把 24kHz 管线误当微信语音管线，导致按住说话不被替换。）
// ============================================================
