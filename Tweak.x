...
%ctor {
    @autoreleasepool {
        ...
        MVLog(@"载入 我的语音 v2.8.40（★ 2.8.40 修「同步服务器音色」同步成功却列表不显示（永远空）：MVServerVoices 读端只读 jbroot plist，而微信沙箱常拒绝写 jbroot、值只落容器 → 读不到；现改为全局域→jbroot→容器三路合并去重，写端补写 CFPreferences 全局域。★ 2.8.39 自建服务器地址自动清洗全角冒号/空格/零宽字符，根治复制地址导致的「不支持的URL」。★ 2.8.38 修复 2.8.37「同步」按钮漏建（点没反应/列表无「服务器音色」节）：buildPickerView 补全 syncBtn 创建并接 onSyncServerVoices；同步成功且当前卡在缺参考音频的克隆音色则自动切到第一个服务器音色。★ 2.8.37 新增「同步服务器音色」：悬浮面板选音色点「同步」拉取电脑 server.py 的 voices/ 语音包与模型预置，选中免上传参考音频即用；★ 2.8.36 修自建服务器地址漏 http:// 时合成报「不支持的URL」：MVSelfHostURL 自动补 scheme。 「自建服务器」开关/地址已搬到【系统设置 → 我的语音 → 自建服务器】，悬浮面板点了只显示状态并指路；删除用不到的「对象存储 OSS」整组；并修「设置页改了不生效」—— 自建键改为 jbroot 共享域优先读取，且面板不再写这三个键。★ 2.8.33 新增「自建服务器」开关：本地 CosyVoice + 阿里云 ECS 中转免费克隆音色；开启后 CosyVoice 族走本地 server.py 不再耗额度。★ 2.8.32 修「千问合成失败·url error!」：合成端点改为按【模型家族】分流 —— Qwen-TTS(qwen3-tts-*) 走 multimodal-generation，Qwen-Audio-TTS(qwen-audio-3.0-tts-*) / CosyVoice 走 /services/audio/tts/SpeechSynthesizer；旧版按 provider 分流，千问克隆音色被送错端点 → 服务端 400 url error，与充值/额度无关。同时修好预置音色选「可调版」不生效、报错信息带端点）");
        MVLog(@"宿主 App：%@（版本 %@）",
