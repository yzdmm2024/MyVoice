...
%ctor {
    @autoreleasepool {
        ...
        //   结论：%ctor 内**只允许**做与宿主完全无关的事（写日志、入队）。
                MVLog(@"载入 我的语音 v2.8.41（★ 2.8.41 移除「自建服务器」通道：本地 CosyVoice + 阿里云 ECS 中转方案整体下线 —— 手机端点一次试听要等近 3 分钟才出声，体验不可接受。删除项：悬浮面板的「同步」按钮与「同步服务器音色」、系统设置里的服务器地址 / 访问令牌、ttsChannel 的第 0 档；老用户存过的 ttsChannel=0 会自动归并到「云端 CosyVoice」，不会卡在无效档位。发音通道现为二选一：云端 CosyVoice（克隆 / 设计音色）与云端千问（预置音色）。 ★ 2.8.32 修「千问合成失败·url error!」：合成端点改为按【模型家族】分流 —— Qwen-TTS(qwen3-tts-*) 走 multimodal-generation，Qwen-Audio-TTS(qwen-audio-3.0-tts-*) / CosyVoice 走 /services/audio/tts/SpeechSynthesizer；旧版按 provider 分流，千问克隆音色被送错端点 → 服务端 400 url error，与充值/额度无关。同时修好预置音色选「可调版」不生效、报错信息带端点）");
        MVLog(@"宿主 App：%@（版本 %@）",
