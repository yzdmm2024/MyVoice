...
- (NSArray<NSDictionary*>*)voiceList {
    ...
    // ★ 2.2.10：CosyVoice 侧把内置方言音色（声音设计生成）排在用户自建音色之前
    return [MVCosyDialectList() arrayByAddingObjectsFromArray:MVVoices()];
}
...
- (void)buildPanel {
    ...
    // ★ 2.2.10：键盘"有时不弹出"的根因——面板拖动手势默认 cancelsTouchesInView=YES，
    //   手指在 textView 上轻微移动会被它取消 textView 的触摸 → textView 没成为第一响应者 → 键盘不弹。
    //   设 NO 后触摸照常送达 textView（点一下就弹键盘），拖动依旧由 pan 处理。
...
- (void)buildHomeView {
    ...
    p2.cancelsTouchesInView = NO; p2.delaysTouchesBegan = NO;
...
- (void)refreshVoiceState {
    ...
    // ★ 2.2.10：分组标题/说明随引擎切换（方言音色只在 CosyVoice 侧）
    if (self.sectionLabel) {
        self.sectionLabel.text = (MVTTSProvider() == 1)
            ? @"千问（预置音色·支持语气/语速）"
            : @"CosyVoice（含方言·点选即生成）";
    }
    if (self.pickerHintLabel) {
        self.pickerHintLabel.text = (MVTTSProvider() == 1)
            ? @"语速/语气只对千问音色生效"
            : @"方言音色首次点选会生成（约10~30秒），之后长期可用";
    }
}
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath {
    ...
    NSString *vid = [self effectiveVoiceIDFor:d];
    if (d[@"design"] && !vid.length) {
        cell.detailTextLabel.text = @"点此一键生成（约10~30秒）";
        cell.detailTextLabel.textColor = [UIColor systemBlueColor];
    } else {
        cell.detailTextLabel.text = [NSString stringWithFormat:@"ID: %@ · model: %@", vid ?: @"", d[@"model"] ?: @""];
        cell.detailTextLabel.textColor = [UIColor secondaryLabelColor];
    }
    cell.accessoryType = (vid.length && [vid isEqualToString:self.selectedVoiceID]) ? UITableViewCellAccessoryCheckmark : UITableViewCellAccessoryNone;
    return cell;
}
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath {
    ...
    NSDictionary *d = self.filteredVoices[(NSUInteger)indexPath.row];
    // ★ 2.2.10：未生成的方言音色 → 点选即生成，不在本行直接选中
    if (d[@"design"] && ![self effectiveVoiceIDFor:d]) {
        [self generateDialect:d];
        return;
    }
    NSString *vid = [self effectiveVoiceIDFor:d];
    if (!vid.length) { [[MyVoiceManager shared] toast:@"该音色尚未就绪"]; return; }
    self.selectedVoiceID = vid;
    if (MVTTSProvider() == 1) {
...
#pragma mark - 方言音色生成（2.2.10）

// 取该条目真正用于合成的 voice_id：方言音色若还没生成则返回 nil
- (NSString*)effectiveVoiceIDFor:(NSDictionary*)d {
    if (d[@"design"]) {
        NSString *real = MVDialectRealVoiceID(d[@"voiceKey"]);
        return real.length ? real : nil;
    }
    return d[@"voiceID"];
}

// 方言音色首次点选 → 用 CosyVoice 声音设计生成专属 voice_id（约 10~30 秒）
- (void)generateDialect:(NSDictionary*)d {
    if (!MVAPIKey().length) {
        [[MyVoiceManager shared] toast:@"请先在 设置→我的语音 填写 DashScope API Key"];
        return;
    }
    NSString *name = d[@"name"] ?: @"方言音色";
    [[MyVoiceManager shared] toast:[NSString stringWithFormat:@"正在生成%@…约10~30秒", name]];
    [[MyVoiceCloud shared] designVoiceWithName:name prompt:d[@"prompt"] previewText:d[@"preview"]
        completion:^(NSString *vid, NSError *err){
            dispatch_async(dispatch_get_main_queue(), ^{
                if (!vid) {
                    [[MyVoiceManager shared] toast:[NSString stringWithFormat:@"%@生成失败：%@",
                        name, err.localizedDescription ?: @"未知"]];
                    [self.voiceTable reloadData];
                    return;
                }
                MVSetDialectRealVoiceID(d[@"voiceKey"], vid);
                self.selectedVoiceID = vid;
                MVSetShared(@"currentVoiceID", vid);
                [self refreshVoiceState];
                [[MyVoiceManager shared] toast:[NSString stringWithFormat:@"✅ %@已生成，可直接发送", name]];
            });
        }];
}
...
- (void)addDrag:(UIButton*)btn {
    ...
    p.cancelsTouchesInView = NO; p.delaysTouchesBegan = NO;
